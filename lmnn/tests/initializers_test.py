...
# Import should be able to retreive all initializers more conviniently
from lmnn.initializers.functions import HeInitializer, RandomInitializer, XavierInitializer, InitializerStruct
...
class TestHeInitializer(unittest.TestCase):
    ...
    def test_generate_weights_distribution(self):
        ...
        self.assertAlmostEqual(np.mean(weights), 0, places=1)
...
class TestRandomInitializer(unittest.TestCase):
    ...
    def test_generate_weights_distribution_classic(self):
        ...
        self.assertAlmostEqual(np.std(weights), 1, places=1)
    ...
    def test_generate_weights_distribution_small(self):
        ...
        self.assertAlmostEqual(np.mean(weights), 0.5, places=1)
        
        # Check stddev is approximately sqrt(1/12) for uniform distribution
        expected_stddev = np.sqrt(1 / 12)
        self.assertAlmostEqual(np.std(weights), expected_stddev, places=1)
